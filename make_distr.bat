...
@set DISTR_NAME=marty-yaml2json
...
@rem xcopy /Y /S /E /I /F /R     ..\umba-brief-scanner\_distr_conf\conf\*                                             %TARGET_ROOT%\conf
@rem copy /Y                     ..\umba-pretty-headers\_distr_conf\conf\umba-pretty-headers.custom.options           %TARGET_ROOT%\conf\umba-pretty-headers.custom.options.example

@copy %BUILD_OUTPUT%\_qt_stub.exe              %TARGET_ROOT%\bin\_qt_stub.exe
@copy %BUILD_OUTPUT%\marty-json-diff.exe       %TARGET_ROOT%\bin\marty-json-diff.exe  
@copy %BUILD_OUTPUT%\marty-json-patch.exe      %TARGET_ROOT%\bin\marty-json-patch.exe 
@copy %BUILD_OUTPUT%\marty-json2yaml.exe       %TARGET_ROOT%\bin\marty-json2yaml.exe  
@copy %BUILD_OUTPUT%\marty-yaml2json.exe       %TARGET_ROOT%\bin\marty-yaml2json.exe  


@set VERSION=1.0
@rem %BUILD_OUTPUT%\%MAIN_EXE_NAME%.exe -v >version.txt
...
del %TARGET_ROOT%\bin\_qt_stub.exe
